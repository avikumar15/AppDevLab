...
public class DiceView extends View {
    ...
    public void setInterface(DiceInterface mDiceInterface) {
        this.mDiceInterface = mDiceInterface;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawBitmap(bitmaps.get((currentDice) % 6), (float) (diceViewEdge / 2f + x), diceViewEdge / 2f, null);

        Log.i(TAG, "bitmaps length is " + bitmaps.size());

        if (isReady) {

            t += RATE_OF_OSCILLATION;
            x = (diceViewEdge / 2f) * Math.sin(t);

            // HACK TO CHANGE THE BITMAP
            // SINCE x might not be exactly equal to 0
            if ((2 * x / diceViewEdge) < 0.2 && (2 * x / diceViewEdge) > -0.2) {

                if (isInTransition)
                    currentDice++;

                // To make sure currentDice is incremented only once
                isInTransition = false;

                if (value == currentDice % 6){
                    frequency--;
                    if (frequency == 0) {
                        isReady = false;
                        mDiceInterface.onRolled(value);
                    }
                }
        ...
        Log.i(TAG, "The value of x: " + x + " and currentDice: " + currentDice);
        Log.i(TAG, "The value of frequency: " + frequency + " and value: " + value);
        invalidate();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int widthXml = MeasureSpec.getSize(widthMeasureSpec);
    ...
    private void setBitmap() {
        bitmaps.add(Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.one),
                diceViewEdge, diceViewEdge, false));
        bitmaps.add(Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.two),
                diceViewEdge, diceViewEdge, false));
        bitmaps.add(Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.three),
                diceViewEdge, diceViewEdge, false));
        bitmaps.add(Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.four),
                diceViewEdge, diceViewEdge, false));
        bitmaps.add(Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.five),
                diceViewEdge, diceViewEdge, false));
        bitmaps.add(Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.six),
                diceViewEdge, diceViewEdge, false));
        isBitmapSet = true;
    }
